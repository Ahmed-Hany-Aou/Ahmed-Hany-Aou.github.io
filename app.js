...
document.addEventListener("DOMContentLoaded", function () {
  ...
  class CVAnalytics {
    constructor() {
      this.downloadCount = parseInt(
        localStorage.getItem("cv_download_count") || "0",
      );
      this.downloadHistory = JSON.parse(
        localStorage.getItem("cv_download_history") || "[]",
      );
      this.updateDisplays();
    }
  ...
  class CVDownloadManager {
    ...
    async downloadCV(format = "pdf", button = null) {
      const downloadUrl =
        format === "docx" ? config.cvWordUrl : config.cvDownloadUrl;
  ...
  class ShareManager {
    ...
    shareToPlatform(platform) {
      ...
      const description =
        "Senior Laravel Backend Engineer specializing in scalable SaaS solutions and modern admin interfaces.";
  ...
  // ============================================
  // GENERIC CARD PAGINATOR
  // ============================================

  class CardPaginator {
    constructor(cardsSelector, paginationContainerId) {
      ...
      if (this.paginationContainer) {
        ...
        this.currentPageSpan =
          this.paginationContainer.querySelector(".current-page");
        this.totalPagesSpan =
          this.paginationContainer.querySelector(".total-pages");
      }
    ...
    getVisibleCards() {
      const allCards = document.querySelectorAll(this.cardsSelector);
    ...
    changePage(direction) {
      const activeCards = this.getVisibleCards();
      const totalPages = Math.ceil(activeCards.length / this.itemsPerPage);
    ...
    update() {
      ...
      allCards.forEach((card) => {
        ...
        // Check if card is within the current page's index range in active list
  ...
  form.addEventListener("submit", (e) => {
    ...
    const whatsappUrl = `https://wa.me/201065232774?text=${encodedText}`;

    // Show loading state
    submitBtn.classList.add("loading");
    submitBtn.disabled = true;

    // Attempt to open WhatsApp immediately in a new tab (synchronous, bypasses popup blocker)
    ...
    submitBtn.querySelector(".btn-text").innerHTML =
      '<i class="fab fa-whatsapp"></i> WhatsApp Opened!';
  ...
  try {
    ...
    const hostedLinksPaginator = new CardPaginator(
      "#hosted-links .project-card",
      "hosted-links-pagination",
    );
    ...
    console.log(
      "✅ Ahmed Hany Boshra Portfolio - Enhanced version loaded successfully!",
    );
    console.log("📄 CV Download and sharing features are ready");
    console.log(
      "🎯 Filament Forever - Building the future, one line at a time.",
    );
  } catch (error) {
